...
import { Split, Plus, Search, CheckCircle } from 'lucide-react'
import { format } from 'date-fns'
import { ptBR } from 'date-fns/locale'
import Layout from '@/components/Layout'
import { api } from '@/lib/api'

interface UntanglingEntry {
  id: string
  op_number: string
  client: string
  color: string
...
export default function Destrinchagem() {
  ...
  const [form, setForm] = useState({ op_id: '', operator: '', quantity_kg: '', observations: '' })

  const { data: entries = [], isLoading } = useQuery<UntanglingEntry[]>({
    queryKey: ['untangling'],
    queryFn: () => api.get('/untangling').then(r => r.data),
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof form) => api.post('/untangling', data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['untangling'] }); toast.success('Lançado na destrinchagem!'); setShowModal(false) },
    onError: () => toast.error('Erro ao lançar'),
  })

  const finishMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/untangling/${id}/finish`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['untangling'] }); toast.success('Destrinchagem finalizada!') },
  ...
  const statusMap: Record<string, { label: string; color: string }> = {
    in_progress: { label: 'Em Andamento', color: 'bg-pink-100 text-pink-700' },
      ...
      <div className="h-full flex flex-col overflow-hidden">
        <div className="flex-shrink-0 flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <Split className="w-6 h-6 text-pink-600" /> Destrinchagem
            </h1>
            <p className="text-gray-600 text-sm">Controle de destrinchagem por OP</p>
          </div>
        ...
        <div className="flex-shrink-0 mb-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar por OP, cliente ou operador..."
              value={search} onChange={e => setSearch(e.target.value)}
              className="pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 w-full md:w-80" />
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-auto">
            {isLoading ? (
              <div className="flex items-center justify-center h-40"><div className="w-8 h-8 border-4 border-pink-500 border-t-transparent rounded-full animate-spin" /></div>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>{['OP','Cliente','Cor','Operador','Qtd (kg)','Status','Início','Término','Ações'].map(h => (
                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  }) : <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-500">Nenhum registro</td></tr>}
                </tbody>
              </table>
            )}
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-gray-200"><h3 className="text-xl font-bold text-gray-900">Lançar Destrinchagem</h3></div>
            <form onSubmit={e => { e.preventDefault(); createMutation.mutate(form) }} className="p-6 space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button type="submit" disabled={createMutation.isPending} className="px-6 py-2 text-sm font-medium text-white bg-pink-600 hover:bg-pink-700 rounded-lg disabled:opacity-50">
