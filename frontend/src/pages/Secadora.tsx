...
import { Wind, Plus, Search, CheckCircle } from 'lucide-react'
...
export default function Secadora() {
  ...
  const [form, setForm] = useState({ op_id: '', machine: '', temperature: '', time_minutes: '', quantity_kg: '', observations: '' })

  const { data: entries = [], isLoading } = useQuery<DryerEntry[]>({
    queryKey: ['dryer'],
    queryFn: () => api.get('/dryer').then(r => r.data),
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof form) => api.post('/dryer', data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['dryer'] }); toast.success('Lançado na secadora!'); setShowModal(false) },
    onError: () => toast.error('Erro ao lançar'),
  })

  const finishMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/dryer/${id}/finish`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['dryer'] }); toast.success('Secagem finalizada!') },
        ...
        <div className="flex-shrink-0 flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <Wind className="w-6 h-6 text-orange-500" /> Secadora
            </h1>
            <p className="text-gray-600 text-sm">Controle de secagem por OP</p>
          </div>
          <button onClick={() => setShowModal(true)} className="flex items-center space-x-2 bg-red-900 hover:bg-red-950 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm">
            <Plus className="w-4 h-4" /><span>Lançar Secagem</span>
          </button>
        </div>
        ...
        <div className="flex-shrink-0 mb-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar por OP ou cliente..."
              ...
              className="pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 w-full md:w-80" />
          </div>
        </div>
            ...
            {isLoading ? (
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>{['OP','Cliente','Cor','Máquina','Qtd (kg)','Temp (°C)','Tempo (min)','Status','Ações'].map(h => (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-gray-200"><h3 className="text-xl font-bold text-gray-900">Lançar Secagem</h3></div>
            <form onSubmit={e => { e.preventDefault(); createMutation.mutate(form) }} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[['Máquina','machine','text'],['Qtd (kg)','quantity_kg','number'],['Temperatura (°C)','temperature','number'],['Tempo (min)','time_minutes','number']].map(([label, key, type]) => (
                  <div key={key}>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">{label} *</label>
                    <input type={type} value={(form as any)[key]} onChange={e => setForm(f => ({...f, [key]: e.target.value}))} required
                      step={key === 'quantity_kg' ? '0.01' : undefined}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 text-sm" />
                  </div>
                ))}
              </div>
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button type="submit" disabled={createMutation.isPending} className="px-6 py-2 text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 rounded-lg disabled:opacity-50">
