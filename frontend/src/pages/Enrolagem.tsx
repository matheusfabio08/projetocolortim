...
import { ScrollText, Plus, Search, CheckCircle } from 'lucide-react'
...
export default function Enrolagem() {
  ...
  const [form, setForm] = useState({ op_id: '', operator: '', rolls_count: '', quantity_kg: '', observations: '' })

  const { data: entries = [], isLoading } = useQuery<RollingEntry[]>({
    queryKey: ['rolling'],
    queryFn: () => api.get('/rolling').then(r => r.data),
  })
  ...
  const createMutation = useMutation({
    mutationFn: (data: typeof form) => api.post('/rolling', data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['rolling'] }); toast.success('Lançado na enrolagem!'); setShowModal(false) },
    onError: () => toast.error('Erro ao lançar'),
  })

  const finishMutation = useMutation({
    mutationFn: (id: string) => api.patch(`/rolling/${id}/finish`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['rolling'] }); toast.success('Enrolagem finalizada!') },
  ...
  const statusMap: Record<string, { label: string; color: string }> = {
    in_progress: { label: 'Em Andamento', color: 'bg-cyan-100 text-cyan-700' },
      ...
      <div className="h-full flex flex-col overflow-hidden">
        <div className="flex-shrink-0 flex items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
              <ScrollText className="w-6 h-6 text-cyan-600" /> Enrolagem
            </h1>
            <p className="text-gray-600 text-sm">Controle de enrolagem por OP</p>
          </div>
        ...
        <div className="flex-shrink-0 mb-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar por OP ou cliente..."
              ...
              className="pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 w-full md:w-80" />
          </div>
        </div>
        ...
        <div className="flex-1 min-h-0 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col overflow-hidden">
          <div className="flex-1 overflow-auto">
            {isLoading ? (
              <div className="flex items-center justify-center h-40"><div className="w-8 h-8 border-4 border-cyan-500 border-t-transparent rounded-full animate-spin" /></div>
            ) : (
              <table className="w-full text-sm">
                <thead className="bg-gray-50 sticky top-0">
                  <tr>{['OP','Cliente','Cor','Operador','Rolos','Qtd (kg)','Status','Início','Término','Ações'].map(h => (
                ...
                <tbody className="divide-y divide-gray-200">
                  {filtered.length > 0 ? filtered.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 tabular-nums">{e.rolls_count}</td>
                        ...
                        <td className="px-3 py-2 text-gray-500 whitespace-nowrap">{e.started_at ? format(new Date(e.started_at), 'dd/MM HH:mm', { locale: ptBR }) : '-'}</td>
                        <td className="px-3 py-2 text-gray-500 whitespace-nowrap">{e.finished_at ? format(new Date(e.finished_at), 'dd/MM HH:mm', { locale: ptBR }) : '-'}</td>
                  ...
                  }) : <tr><td colSpan={10} className="px-3 py-8 text-center text-gray-500">Nenhum registro</td></tr>}
                </tbody>
              </table>
            )}
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="p-6 border-b border-gray-200"><h3 className="text-xl font-bold text-gray-900">Lançar Enrolagem</h3></div>
            <form onSubmit={e => { e.preventDefault(); createMutation.mutate(form) }} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2">
                  ...
                  <input type="text" value={form.operator} onChange={e => setForm(f => ({...f, operator: e.target.value}))} required
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Nº Rolos</label>
                  <input type="number" min="1" value={form.rolls_count} onChange={e => setForm(f => ({...f, rolls_count: e.target.value}))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 text-sm" />
                </div>
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.observations} onChange={e => setForm(f => ({...f, observations: e.target.value}))} rows={2}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-500 text-sm resize-none" />
              </div>
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button type="submit" disabled={createMutation.isPending} className="px-6 py-2 text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-700 rounded-lg disabled:opacity-50">
