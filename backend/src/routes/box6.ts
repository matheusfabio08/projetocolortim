...
router.get('/records', async (_req, res: Response) => {
  const [waiting, inProgressRaw] = await Promise.all([
    ...
    prisma.poInProgress.findMany({ where: { stage: 'box6' }, include: { op: true } }),
  ]);
...
router.post('/', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  await prisma.productionOrder.update({ where: { id: po_id }, data: { status: 'producao', currentStage: 'box6' } });
  await prisma.activityLog.create({ data: { opId: po_id, stage: 'box6', action: 'processed', userId: req.user!.id } });
