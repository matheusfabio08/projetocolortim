...
export default function Box6() {
  ...
  const fetchEmployees = async () => {
    try {
      const data = await request("/api/employees?sector=Box 6");
    ...
    try {
      ...
      if (ops && ops.length > 0) {
        const op = ops.find((o: ProductionOrder) => 
          o.op_number === opSearch && o.status === "box6"
        );
        if (op) {
          setSelectedOP(op);
          
          const status = await request(`/api/op-status/${op.id}/box6`);
          setIsInProgress(status.in_progress);
          setShowForm(false);
          setView("process");
        } else {
          alert("OP não encontrada ou não está no status Box 6");
        }
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Box 6 concluído com sucesso!");
      setSelectedOP(null);
      setOpSearch("");
      setEmployee("");
    ...
    <Layout>
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Box 6</h1>
            <p className="text-gray-600">Registrar processamento no Box 6</p>
          </div>
